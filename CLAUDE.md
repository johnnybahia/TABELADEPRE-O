...
**O mesmo layout/ERP atende dois compradores** — ANIGER (Quixeramobim/CE) e MELBROS (Campo Bom/RS) — e o texto de "condições gerais" dos dois cita as duas razões sociais. Por isso a identificação do cliente é feita pelo **CNPJ do topo do pedido** (`CONF_ANIGER_COMPRADOR`, chaveado pela raiz de 8 dígitos → `{uf, cliente}`) e **ganha da busca por nome no texto** (`campos.clienteHint` agora tem precedência em `confArquivosSelecionados`; sem CNPJ conhecido o card fica sem cliente para o usuário escolher, em vez de conferir contra a tabela errada).
...
- **Modalidade origem→destino**: origem = CNPJ da filial Marfim no bloco do fornecedor; destino = CNPJ do comprador, com o **CEP do bloco do comprador** como reserva para filial ainda não mapeada (9xxxx = RS · 60000-63999 = CE). Nas amostras: `CE→CE` (2), `RS→CE` (1) e `RS→RS` (1, Melbros).
